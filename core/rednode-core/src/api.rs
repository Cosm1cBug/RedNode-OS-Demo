...
async fn health() -> Json<serde_json::Value> {
    ...
    let nats_connected = crate::bus::get_client().is_some();
    let db_connected = crate::memory::pool().is_some();
    Json(serde_json::json!({
        ...
        "uptime_secs": uptime,
        "services": {
            "nats": if nats_connected { "connected" } else { "disconnected" },
            "postgres": if db_connected { "connected" } else { "disconnected" },
        },
        "degraded": !nats_connected || !db_connected
    }))
}
...
pub fn router() -> Router {
    Router::new()
        .route("/health", get(health)).route("/intent", post(intent_handler)).route("/events", get(ws_handler))
        .route("/audit", get(audit_log)).route("/approvals", get(list_approvals)).route("/approvals/:id/approve", post(approve_handler))
        .route("/memory/query", get(memory_query)).route("/memory/ingest", post(memory_ingest))
        .route("/security/events", get(security_events).post(security_event_post)).route("/security/events/:id/ack", post(security_event_ack))
        .route("/agents/status", get(agents_status)).route("/sentience", get(sentience_status))
        .route("/kg/query", get(kg_query_handler)).route("/kg/entity", post(kg_add_entity_handler))
        .route("/evolve/tool", post(evolve_tool_handler)).route("/evolve/tools", get(list_evolved_tools))
        .route("/config", get(config_dashboard)).route("/config/agent", get(config_agent)).route("/config/setup", get(config_setup_status))
        .route("/config/preferences", post(config_update_preferences)).route("/config/secret", post(config_set_secret))
        .route("/config/:service", get(config_get_service).post(config_update_service)).route("/config/test/:service", post(config_test_service))
        .route("/consciousness", get(consciousness_status)).route("/consciousness/summary", get(consciousness_summary))
        .route("/goals", get(goals_list).post(goals_create)).route("/goals/:id", get(goals_get).delete(goals_delete))
        .route("/goals/:id/subgoal", post(goals_add_subgoal)).route("/goals/:id/contribute", post(goals_contribute))
        .route("/world", get(world_full)).route("/world/summary", get(world_summary))
        .route("/world/machines", get(world_machines_list).post(world_machine_upsert)).route("/world/machines/:id", get(world_machine_get).delete(world_machine_delete))
        .route("/world/services", get(world_services_list).post(world_service_upsert)).route("/world/topology", get(world_topology))
        .route("/world/dependencies/:id", get(world_dependencies)).route("/world/impact/:id", get(world_impact))
        .route("/world/diff", get(world_diff)).route("/world/edges", post(world_add_edge)).route("/world/scan", post(world_scan))
        .route("/time", get(time_awareness)).route("/time/summary", get(time_summary))
        .route("/time/events", get(time_events_list).post(time_event_create)).route("/time/events/:id", delete(time_event_delete))
        .route("/time/patterns", get(time_patterns_list).post(time_pattern_create)).route("/time/patterns/:id", delete(time_pattern_delete))
        .route("/time/deadlines", get(time_deadlines_list).post(time_deadline_create)).route("/time/due", get(time_due_items))
        .route("/personality", get(personality_get).post(personality_update)).route("/personality/prompt", get(personality_prompt))
        .route("/reflection/today", get(reflection_today)).route("/reflection/history", get(reflection_history)).route("/reflection/trigger", post(reflection_trigger))
        .route("/curiosity", get(curiosity_status)).route("/curiosity/discoveries", get(curiosity_discoveries))
        .route("/curiosity/config", post(curiosity_config_update)).route("/curiosity/explore", post(curiosity_explore))
        .route("/distillation", get(distillation_list)).route("/distillation/search", get(distillation_search))
        .route("/distillation/trigger", post(distillation_trigger)).route("/distillation/:id", get(distillation_get))
        .route("/economy", get(economy_status)).route("/economy/history", get(economy_history)).route("/economy/budget", post(economy_set_budget))
        .route("/immune", get(immune_status)).route("/immune/threats", get(immune_threats)).route("/immune/scan", post(immune_scan))
        .route("/governance/policies", get(governance_policies).post(governance_create_policy)).route("/governance/policies/:id", delete(governance_delete_policy))
        .route("/governance/violations", get(governance_violations)).route("/governance/check", post(governance_check))
        .route("/plugins", get(plugins_list)).route("/plugins/install", post(plugins_install))
        .route("/plugins/:id", get(plugins_get).delete(plugins_uninstall)).route("/plugins/:id/enable", post(plugins_enable))
        .route("/twin/simulate", post(twin_simulate)).route("/twin/history", get(twin_history)).route("/twin/validate", post(twin_validate))
        .route("/identity", get(identity_get).post(identity_update)).route("/identity/purpose", get(identity_purpose))
        .route("/identity/principles", get(identity_principles)).route("/identity/boundaries", get(identity_boundaries))
        .route("/identity/capabilities", get(identity_capabilities)).route("/identity/prompt", get(identity_prompt))
        .route("/constitution", get(constitution_get)).route("/constitution/check", post(constitution_check))
        .route("/constitution/violations", get(constitution_violations)).route("/constitution/amend", post(constitution_amend))
        .route("/meta/analysis", get(meta_analysis)).route("/meta/strategies", get(meta_strategies)).route("/meta/suggestions", get(meta_suggestions))
        .route("/memory/episodic", get(memory_episodic)).route("/memory/procedural", get(memory_procedural)).route("/memory/working", get(memory_working))
        .route("/simulate/plan", post(simulate_plan)).route("/simulate/history", get(simulate_history))
        .route("/debate/start", post(debate_start)).route("/debate/history", get(debate_history))
        .route("/trust", get(trust_all)).route("/trust/:source", get(trust_get).post(trust_set))

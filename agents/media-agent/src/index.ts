...
const TOOLS = [
  "media.search", "media.library", "media.playing", "media.play",
  "media.pause", "media.recent", "media.sessions",
];
...
class MediaAgent extends RedNodeAgent {
  constructor() { super("media", TOOLS); }

  async handleTool(tool: string, args: any): Promise<any> {
    if (!JELLYFIN_API_KEY) {
      return { ok: false, error: "Jellyfin not configured. Set JELLYFIN_URL and JELLYFIN_API_KEY." };
    }
    ...
    try {
      switch (tool) {
        case "media.search": {
          ...
          const data = await jellyfinGet(`/Items?searchTerm=${encodeURIComponent(query)}&Limit=10&Recursive=true&api_key=${JELLYFIN_API_KEY}`);
          const items = (data.Items || []).map((i: any) =>
            `${i.Type}: ${i.Name}${i.ProductionYear ? ` (${i.ProductionYear})` : ""}${i.SeriesName ? ` — ${i.SeriesName}` : ""}`
          );
          return { ok: true, output: items.length > 0 ? items.join("\n") : `No results for: "${query}"`, count: items.length };
        }

        case "media.library": {
          const data = await jellyfinGet(`/Library/VirtualFolders?api_key=${JELLYFIN_API_KEY}`);
          const libs = (data || []).map((l: any) => `📁 ${l.Name} (${l.CollectionType || "mixed"}) — ${l.Locations?.join(", ") || "?"}`);
          return { ok: true, output: libs.join("\n") || "No libraries configured" };
        }

        case "media.recent": {
          const data = await jellyfinGet(`/Items/Latest?Limit=10&api_key=${JELLYFIN_API_KEY}`);
          const items = (data || []).map((i: any) =>
            `${i.Type === "Episode" ? "📺" : i.Type === "Movie" ? "🎬" : "🎵"} ${i.Name}${i.SeriesName ? ` (${i.SeriesName})` : ""}`
          );
          return { ok: true, output: items.length > 0 ? `Recently added:\n${items.join("\n")}` : "No recent items" };
        }

        case "media.sessions": {
          const data = await jellyfinGet(`/Sessions?api_key=${JELLYFIN_API_KEY}`);
          const active = (data || []).filter((s: any) => s.NowPlayingItem);
          if (active.length === 0) return { ok: true, output: "Nothing currently playing" };
        ...
        case "media.pause": {
          ...
          return { ok: true, output: `Media playback control: use the Jellyfin app or web UI at ${JELLYFIN_URL}. Remote play/pause requires session management.` };
        }

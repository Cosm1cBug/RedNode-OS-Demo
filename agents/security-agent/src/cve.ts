...
async function getInstalledPackages(): Promise<Package[]> {
  ...
  try {
    ...
    for (const line of stdout.trim().split("\n")) {
      ...
      if (name && name.length > 0) {
        pkgs.push({ name: name.trim(), version: (version || "unknown").trim(), manager: "dpkg" });
      }
    }
  } catch {}
  ...
  if (pkgs.length === 0) {
    try {
      ...
      for (const line of stdout.trim().split("\n")) {
        const [name, version] = line.split(" ");
        if (name && name.length > 0) {
          pkgs.push({ name: name.trim(), version: (version || "unknown").trim(), manager: "rpm" });
        }
      }
    } catch {}
  }
  ...
  if (pkgs.length === 0) {
    ...
    console.log(`[cve] Package inventory: ${pkgs.length} packages (${pkgs[0]?.manager})`);
  }
...
function loadCveDb() {
  ...
  cveDb = [
    { cve: "CVE-2024-5535", pkg: "openssl", affected_before: "3.0.14", fixed: "3.0.14", severity: "HIGH", summary: "OpenSSL SSL_select_next_proto buffer overread" },
    { cve: "CVE-2024-6119", pkg: "openssl", affected_before: "3.0.15", fixed: "3.0.15", severity: "MEDIUM", summary: "OpenSSL possible DoS in X.509 name checks" },
    { cve: "CVE-2024-2961", pkg: "glibc", affected_before: "2.39", fixed: "2.39", severity: "HIGH", summary: "glibc iconv buffer overflow in ISO-2022-CN-EXT" },
    { cve: "CVE-2024-47176", pkg: "cups", affected_before: "2.4.11", fixed: "2.4.11", severity: "CRITICAL", summary: "CUPS cups-browsed binds to UDP 631 on all interfaces — RCE chain" },
    { cve: "CVE-2024-3094", pkg: "xz-utils", affected_before: "5.6.2", fixed: "5.6.2", severity: "CRITICAL", summary: "xz/liblzma backdoor — supply chain compromise" },
    { cve: "CVE-2023-44487", pkg: "nginx", affected_before: "1.25.3", fixed: "1.25.3", severity: "HIGH", summary: "HTTP/2 Rapid Reset attack DoS" },
    { cve: "CVE-2024-21626", pkg: "runc", affected_before: "1.1.12", fixed: "1.1.12", severity: "HIGH", summary: "runc container breakout via /proc/self/fd" },
    { cve: "CVE-2024-0567", pkg: "gnutls", affected_before: "3.8.3", fixed: "3.8.3", severity: "MEDIUM", summary: "GnuTLS DoS via crafted certificate chain" },
    { cve: "CVE-2023-50387", pkg: "bind9", affected_before: "9.18.24", fixed: "9.18.24", severity: "HIGH", summary: "BIND KeyTrap — DNSSEC validation CPU exhaustion" },
    { cve: "CVE-2024-6387", pkg: "openssh", affected_before: "9.8", fixed: "9.8p1", severity: "CRITICAL", summary: "regreSSHion — OpenSSH unauthenticated RCE on glibc-based Linux" },
  ];
  ...
  try {
    ...
    console.log(`[cve] Seeded CVE DB with ${cveDb.length} entries → ${CVE_DB_PATH}`);
  } catch {}
}
...
export async function runCveCheck() {
  ...
  if (hits.length === 0) {
    console.log("[security-agent] CVE check: clean — 0 vulnerabilities found ✅");
    await reportSecurityEvent("LOW", "CVE scan completed: 0 vulnerabilities found", {
      status: "clean",
  ...
  console.log(`[security-agent] CVE check: ${hits.length} vulnerable package(s) found ⚠️`);
  for (const h of hits) {
...
console.log(`[security-agent] CVE auto-checker loaded — ${cveDb.length} CVEs in DB — interval 6h`);

...
class ResearchAgent extends RedNodeAgent {
  ...
  async handleTool(tool: string, args: any): Promise<any> {
    switch (tool) {
      ...
      case "rss.fetch": {
        const feedUrl = args.url || args.feed || ""; if (!feedUrl) return { ok: false, error: "Missing RSS feed URL" }; const r = await sh(`curl -sL "${feedUrl}" 2>&1 | head -500`); return { ok: r.ok, output: r.output, tool }; 
      }

      case "rss.digest": {
        const feeds = (process.env.RSS_FEEDS || "").split("|").filter(Boolean); if (!feeds.length) return { ok: true, output: "No RSS feeds configured — set RSS_FEEDS in .env", tool }; const items: string[] = []; for (const feed of feeds.slice(0, 3)) { const r = await sh(`curl -sL "${feed}" 2>&1 | grep -oP "<title>[^<]+" | head -5 | sed "s/<title>//"`, 10000); if (r.ok && r.output) items.push(r.output); } const summary = await llm(`Summarize these news headlines concisely:\n${items.join("\n")}`); return { ok: true, output: summary, tool };
      }

      case "podcast.download": {
        const url = args.url || ""; if (!url) return { ok: false, error: "Missing podcast episode URL" }; const dest = process.env.PODCAST_DOWNLOAD_DIR || "/var/lib/rednode/podcasts"; const r = await sh(`mkdir -p ${dest} && curl -sLo "${dest}/episode-${Date.now()}.mp3" "${url}" 2>&1 && echo "Downloaded to ${dest}"`, 120000); return { ok: r.ok, output: r.output, tool };  
      }

      case "podcast.transcribe": {
        const file = args.file || ""; if (!file) return { ok: false, error: "Missing audio file path" }; const r = await sh(`whisper "${file}" --model small --output_format txt 2>&1 || echo "Whisper not installed — pip install openai-whisper"`, 300000); return { ok: r.ok, output: r.output, tool };  
      }

      case "podcast.summarize": {
        const text = args.text || args.transcript || ""; if (!text) return { ok: false, error: "Missing transcript text" }; const summary = await llm(`Summarize this podcast transcript in 3-5 bullet points:\n${text.substring(0, 3000)}`); return { ok: true, output: summary, tool };  
      }
      ...
      case "research.translate": {
        const text = args.text || ""; const target = args.target || args.language || "English"; if (!text) return { ok: false, error: "Missing text to translate" }; const translated = await llm(`Translate the following to ${target}. Output only the translation:\n${text}`); return { ok: true, output: translated, tool };  
      }

      case "research.summarize_url": {
        const url = args.url || ""; if (!url) return { ok: false, error: "Missing URL" }; const r = await sh(`curl -sL "${url}" 2>&1 | sed "s/<[^>]*>//g" | sed "/^$/d" | head -200`); if (!r.ok) return r; const summary = await llm(`Summarize this webpage content:\n${r.output.substring(0, 3000)}`); return { ok: true, output: summary, tool }; 
      }

      case "research.compare": {
        const a = args.a || args.topic1 || ""; const b = args.b || args.topic2 || ""; if (!a || !b) return { ok: false, error: "Missing two topics to compare" }; const comparison = await llm(`Compare "${a}" vs "${b}" in a structured table format. Cover key differences, pros/cons.`); return { ok: true, output: comparison, tool };  
      }

      case "research.timeline": {
        const topic = args.topic || args.query || ""; if (!topic) return { ok: false, error: "Missing topic" }; const timeline = await llm(`Create a chronological timeline of key events for: "${topic}". Use format: YYYY - Event description.`); return { ok: true, output: timeline, tool };  
      }

      case "research.fact_check": {
        const claim = args.claim || args.statement || ""; if (!claim) return { ok: false, error: "Missing claim to fact-check" }; const analysis = await llm(`Fact-check this claim: "${claim}". Provide: verdict (true/false/partially true), evidence, confidence level.`); return { ok: true, output: analysis, tool }; 
      }

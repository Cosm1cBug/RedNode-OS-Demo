...
const CNS_URL = process.env.REDNODE_URL || process.env.REDNODE_CNS || "http://10.0.50.10:8787";
...
async function getPackages(): Promise<{ name: string; version: string; manager: string }[]> {
  const pkgs: { name: string; version: string; manager: string }[] = [];

  if (PLATFORM === "linux") {
    // dpkg
    try {
      const { stdout } = await execAsync("dpkg-query -W -f='${Package} ${Version}\\n' 2>/dev/null | head -500", { timeout: 10000 });
      for (const line of stdout.trim().split("\n")) {
        const [name, version] = line.split(" ");
        if (name) pkgs.push({ name: name.trim(), version: (version || "?").trim(), manager: "dpkg" });
      }
    } catch {}
    // rpm
    if (pkgs.length === 0) {
      try {
        ...
        for (const line of stdout.trim().split("\n")) {
          const [name, version] = line.split(" ");
          if (name) pkgs.push({ name: name.trim(), version: (version || "?").trim(), manager: "rpm" });
        }
      } catch {}
    }
    // snap
    try {
      const { stdout } = await execAsync("snap list 2>/dev/null | tail -n +2 | head -100", { timeout: 10000 });
      for (const line of stdout.trim().split("\n")) {
        const parts = line.trim().split(/\s+/);
        if (parts[0]) pkgs.push({ name: parts[0], version: parts[1] || "?", manager: "snap" });
      }
    } catch {}
    // flatpak
    try {
      const { stdout } = await execAsync("flatpak list --columns=application,version 2>/dev/null | head -100", { timeout: 10000 });
      for (const line of stdout.trim().split("\n")) {
        const parts = line.trim().split(/\s+/);
        if (parts[0]) pkgs.push({ name: parts[0], version: parts[1] || "?", manager: "flatpak" });
      }
    } catch {}
  }
  ...
  if (PLATFORM === "darwin") {
    // Homebrew
    try {
      const { stdout } = await execAsync("brew list --versions 2>/dev/null | head -200", { timeout: 15000 });
      for (const line of stdout.trim().split("\n")) {
        const parts = line.trim().split(/\s+/);
        if (parts[0]) pkgs.push({ name: parts[0], version: parts.slice(1).join(",") || "?", manager: "brew" });
      }
    } catch {}
  }
  ...
  if (PLATFORM === "win32") {
    ...
    try {
      ...
      for (const p of arr) {
        if (p.Name) pkgs.push({ name: p.Name, version: p.Version || "?", manager: "winget" });
      }
    } catch {}
    // winget
    try {
      const { stdout } = await execAsync("winget list --disable-interactivity 2>nul", { timeout: 30000 });
      for (const line of stdout.trim().split("\n").slice(2)) {
        const parts = line.trim().split(/\s{2,}/);
        if (parts[0] && parts[1]) pkgs.push({ name: parts[0], version: parts[1], manager: "winget" });
      }
    } catch {}
  }
...
async function getOpenPorts(): Promise<{ port: number; process: string }[]> {
  ...
  try {
    if (PLATFORM === "linux") {
      const { stdout } = await execAsync("ss -tlnp 2>/dev/null | tail -n +2", { timeout: 5000 });
      for (const line of stdout.trim().split("\n")) {
        const match = line.match(/:(\d+)\s/);
        const proc = line.match(/users:\(\("([^"]+)"/);
        if (match) ports.push({ port: parseInt(match[1]), process: proc?.[1] || "?" });
      }
    } else if (PLATFORM === "darwin") {
      const { stdout } = await execAsync("lsof -iTCP -sTCP:LISTEN -P -n 2>/dev/null | tail -n +2", { timeout: 5000 });
      for (const line of stdout.trim().split("\n")) {
        const parts = line.trim().split(/\s+/);
        const match = parts[8]?.match(/:(\d+)$/);
        if (match) ports.push({ port: parseInt(match[1]), process: parts[0] || "?" });
      }
    } else if (PLATFORM === "win32") {
      const { stdout } = await execAsync('netstat -ano -p TCP | findstr "LISTENING"', { timeout: 5000 });
      for (const line of stdout.trim().split("\n")) {
        const match = line.match(/:(\d+)\s.*LISTENING\s+(\d+)/);
        if (match) ports.push({ port: parseInt(match[1]), process: `PID:${match[2]}` });
      }
    }
  } catch {}
    ...
    if (PLATFORM === "linux") {
      ...
      for (const line of stdout.trim().split("\n")) {
        ...
        if (parts[0]?.endsWith(".service")) {
          services.push({ name: parts[0].replace(".service", ""), status: "running" });
        }
      }
    } else if (PLATFORM === "darwin") {
      const { stdout } = await execAsync("launchctl list 2>/dev/null | head -50", { timeout: 5000 });
      for (const line of stdout.trim().split("\n").slice(1)) {
        const parts = line.trim().split(/\s+/);
        if (parts[2]) services.push({ name: parts[2], status: parts[0] === "-" ? "running" : `exit:${parts[0]}` });
      }
    } else if (PLATFORM === "win32") {
      const { stdout } = await execAsync(
...
async function getOsVersion(): Promise<string> {
  try {
    if (PLATFORM === "linux") {
      const { stdout } = await execAsync("cat /etc/os-release 2>/dev/null | grep PRETTY_NAME | cut -d'\"' -f2", { timeout: 3000 });
      return stdout.trim() || `Linux ${os.release()}`;
    } else if (PLATFORM === "darwin") {
      const { stdout } = await execAsync("sw_vers -productVersion 2>/dev/null", { timeout: 3000 });
...
async function submitReport(report: EndpointReport): Promise<boolean> {
  try {
    ...
    const pkgSummary = report.packages.slice(0, 50).map(p => `${p.name} ${p.version}`).join(", ");
    const portSummary = report.open_ports.map(p => `${p.port}/${p.process}`).join(", ");
...
async function main() {
  ...
  const report = async () => {
    ...
    console.log(`  ${data.packages.length} packages, ${data.open_ports.length} ports, ${data.services.length} services`);
